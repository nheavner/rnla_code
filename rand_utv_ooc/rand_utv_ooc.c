...
static int Set_to_identity( int m_A, int n_A, double * buff_A,
               int ldim_A );

static int Zero_strict_lower_triangular( int m_A, int n_A,
               double * buff_A, int ldim_A );

static int Zero( int m_A, int n_A, double * buff_A, int ldim_A );

static int Copy_vector_into_diagonal( double * v, int m_A, int n_A,
               double * buff_A, int ldim_A );

static int Multiply_BAB(
               char transa, char transb,
               int m_A, int n_A, double * buff_A, int ldim_A,
               int m_B, int n_B, double * buff_B, int ldim_B );

static int Multiply_BBA(
               char transa, char transb,
               int m_A, int n_A, double * buff_A, int ldim_A,
               int m_B, int n_B, double * buff_B, int ldim_B );

static int Compute_svd(
               int m_A, int n_A, double * buff_A, int ldim_A,
...
static int Apply_Q_WY_lhfc_blk_var2(
               int m_U, int n_U, double * buff_U, int ldim_U,
               int m_T, int n_T, double * buff_T, int ldim_T,
               int m_B, int n_B, double * buff_B, int ldim_B );

static int Apply_Q_WY_rnfc_blk_var2(
               int m_U, int n_U, double * buff_U, int ldim_U,
               int m_T, int n_T, double * buff_T, int ldim_T,
               int m_B, int n_B, double * buff_B, int ldim_B );

static int QRP_WY_unb_var2( int pivoting, int num_stages,
               int m_A, int n_A, double * buff_A, int ldim_A,
               double * buff_T, int ldim_T );

static int QRP_compute_norms(
               int m_A, int n_A, double * buff_A, int ldim_A,
               double * buff_d, double * buff_e );

static int QRP_downdate_partial_norms( int m_A, int n_A,
               double * buff_d,  int st_d,
               double * buff_e,  int st_e,
               double * buff_wt, int st_wt,
               double * buff_A,  int ldim_A );

static int QRP_pivot_G( int j_max_col,
...
//
  ...
  if( build_u == 1 ) {
    ...
    Set_to_identity( m_U, n_U, buff_U, ldim_U );
  }
  if( build_v == 1 ) {
    // MyFLA_Set_to_identity( V );
    Set_to_identity( m_V, n_V, buff_V, ldim_V );
  }
  ...
  for( i = 0; i < mn_A; i += nb_alg ) {
    ...
    Normal_random_matrix( m_GBl, n_GBl, buff_GBl, ldim_G );
    ...
    if( pp > 0 ) {
      ...
      QRP_WY_unb_var2( 1, bRow, m_YBl, n_YBl, buff_YBl, ldim_Y,
          buff_S1tl, ldim_S1 );
    } else {
      // MyFLA_QRP_UT_unb( 0, bRow, YBl, S1tl );
      QRP_WY_unb_var2( 0, bRow, m_YBl, n_YBl, buff_YBl, ldim_Y,
          buff_S1tl, ldim_S1 );
    }
#ifdef PROFILE
    t2 = FLA_Clock();
    ...
    QRP_WY_unb_var2( 0, bRow, m_AB1, n_AB1, buff_AB1, ldim_A,
        buff_S2tl, ldim_S2 );
#ifdef PROFILE
    t2 = FLA_Clock();
    tt_qr2_fact += ( t2 - t1 );
    ...
    Zero_strict_lower_triangular( m_AB1, n_AB1, buff_AB1, ldim_A );
    ...
    Compute_svd(
        ...
        bRow );
    // MyFLA_Zero( A11 );
    Zero( bRow, bRow, buff_A11, ldim_A );
    // MyFLA_Copy_vector_into_diagonal( svl, A11 );
    Copy_vector_into_diagonal( buff_sv, bRow, bRow, buff_A11, ldim_A );
#ifdef PROFILE
    t2 = FLA_Clock();
    tt_svd_fact += ( t2 - t1 );
#endif
...
//https://www.google.com://www.google.com//
  ...
  if( pivoting == 1 ) {
    ...
    QRP_compute_norms( m_A, n_A, buff_A, ldim_A, buff_d, buff_e );
  }
  ...
  for( j = 0; j < num_stages; j++ ) {
    ...
    if( pivoting == 1 ) {
      ...
      QRP_downdate_partial_norms( m_A22, n_A22,
          & buff_d[ j+1 ], 1,
          & buff_e[ j+1 ], 1,
          & buff_A[ j + ( j+1 ) * ldim_A ], ldim_A,
