...
p1 = plot(n_cpqr_ooc(1:length(t_cpqr_in)), t_cpqr_in ./ n_cpqr_ooc(1:length(t_cpqr_in)).^3,'ko-',...
		...
		n_cpqr_ooc_pp(1:length(t_cpqr_in_pp)), t_cpqr_in_pp ./ n_cpqr_ooc_pp(1:length(t_cpqr_in_pp)).^3,'ko--',...
		n_cpqr_ooc_pp, t_cpqr_ssd_pp ./ n_cpqr_ooc_pp.^3, 'rs--',...
		n_cpqr_ooc_pp(1:length(t_cpqr_hdd_pp)), t_cpqr_hdd_pp ./ n_cpqr_ooc_pp(1:length(t_cpqr_hdd_pp)).^3, 'bd--');
ax = gca;
set(ax,'fontsize',16);
set(p1,'LineWidth',2,'MarkerSize',10);
xlabel('n');
ylabel('Time [s] / n^3');
title('Full factorization times, b=128')
legend(	'In Core',...
		'SSD',...
		'HDD',...
		'In Core, pp',...
