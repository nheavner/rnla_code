...
n     = 200;     % Matrix size.
m     = 200;     % Matrix size.
q_max = 3;       % Maximal power iteration parameter to be tested.
...
ylabel('||A - A_k||')
axis(ax)
legend('svds','col piv QR','randUTV: q=0,p=0','randUTV: q=1,p=0','randUTV: q=2,p=0',...
...
xlabel('k')
ylabel('|R(k,k)|')
legend('svd','col piv QR','randUTV: q=1','randUTV: q=2',...
...
title('Frobenius norm of blocks of T')
...
n_iter = 0;              % Parameter in power iteration.
p      = 0;              % Amount of oversampling to be done.
...
xlabel('n')
ylabel('time')
legend('UTV','SVD','QRP') % ,'UTV_gpu','SVD_gpu'
axis tight
title(strcat('Absolute times for various factorizations (q=',num2str(n_iter),' in UTV)'))
...
xlabel('n')
ylabel('time/n^3')
legend('UTV','SVD','QRP') % 'UTVgpu',SVDgpu
axis tight
title(strcat('Times scaled by n^3 for various factorizations (q=',num2str(n_iter),' in UTV)'))
keyboard
